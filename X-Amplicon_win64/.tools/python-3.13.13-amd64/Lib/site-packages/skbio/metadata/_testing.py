...
class MetadataMixinTests:
    ...
    def test_constructor_with_metadata(self):
        ...
        obj = self._metadata_constructor_(metadata={"": "", 123: {"a": "b", "c": "d"}})
    ...
    def test_eq_missing_metadata(self):
        self.assertReallyEqual(
            self._metadata_constructor_(), self._metadata_constructor_()
        )
        self.assertReallyEqual(
            self._metadata_constructor_(), self._metadata_constructor_(metadata={})
        )
        self.assertReallyEqual(
    ...
    def test_metadata_getter(self):
        obj = self._metadata_constructor_(metadata={42: "foo", ("hello", "world"): 43})
    ...
    def test_metadata_setter_invalid_type(self):
        ...
        for md in (None, 0, "a", ("f", "o", "o"), np.array([]), pd.DataFrame()):
            with self.assertRaisesRegex(TypeError, "metadata must be a dict"):
                obj.metadata = md
    ...
    def test_has_metadata(self):
        ...
        self.assertFalse(self._metadata_constructor_(metadata={}).has_metadata())

        self.assertTrue(self._metadata_constructor_(metadata={"": ""}).has_metadata())
        self.assertTrue(
            self._metadata_constructor_(metadata={"foo": 42}).has_metadata()
        )
...
class PositionalMetadataMixinTests:
    ...
    def test_constructor_positional_metadata_len_mismatch(self):
        ...
        with self.assertRaisesRegex(ValueError, r"\(3\).*\(4\)"):
            self._positional_metadata_constructor_(4, positional_metadata=[2, 3, 4])
    ...
    def test_constructor_no_positional_metadata(self):
        ...
        for empty in None, {}, pd.DataFrame():
            obj = self._positional_metadata_constructor_(0, positional_metadata=empty)
        ...
        obj = self._positional_metadata_constructor_(3, positional_metadata=None)
    ...
    def test_constructor_with_positional_metadata_len_zero(self):
        for data in [], (), np.array([]):
            ...
            assert_data_frame_almost_equal(
                obj.positional_metadata, pd.DataFrame({"foo": data}, index=range(0))
            )
    ...
    def test_constructor_with_positional_metadata_len_one(self):
        for data in [2], (2,), np.array([2]):
            ...
            self.assertTrue(obj.has_positional_metadata())
            assert_data_frame_almost_equal(
                obj.positional_metadata, pd.DataFrame({"foo": data}, index=range(1))
            )
    ...
    def test_constructor_with_positional_metadata_len_greater_than_one(self):
        for data in (
            ...
            self.assertTrue(obj.has_positional_metadata())
            assert_data_frame_almost_equal(
                obj.positional_metadata, pd.DataFrame({"foo": data}, index=range(9))
            )

    def test_constructor_with_positional_metadata_multiple_columns(self):
        obj = self._positional_metadata_constructor_(
            5, positional_metadata={"foo": np.arange(5), "bar": np.arange(5)[::-1]}
        )
        ...
        assert_data_frame_almost_equal(
            ...
            pd.DataFrame(
                {"foo": np.arange(5), "bar": np.arange(5)[::-1]}, index=range(5)
            ),
        )

    def test_constructor_with_positional_metadata_custom_index(self):
        df = pd.DataFrame(
            {"foo": np.arange(5), "bar": np.arange(5)[::-1]},
            index=["a", "b", "c", "d", "e"],
        )
    ...
    def test_constructor_handles_missing_positional_metadata_efficiently(self):
        ...
        obj = self._positional_metadata_constructor_(4, positional_metadata=None)
        self.assertIsNone(obj._positional_metadata)

    def test_constructor_makes_shallow_copy_of_positional_metadata(self):
        df = pd.DataFrame(
            {"foo": [22, 22, 0], "bar": [[], [], []]}, index=["a", "b", "c"]
        )
        obj = self._positional_metadata_constructor_(3, positional_metadata=df)

        assert_data_frame_almost_equal(
            obj.positional_metadata,
            pd.DataFrame({"foo": [22, 22, 0], "bar": [[], [], []]}, index=range(3)),
        )
    ...
    def test_eq_missing_positional_metadata(self):
        for empty in None, {}, pd.DataFrame(), pd.DataFrame(index=[]):
            ...
            self.assertReallyEqual(obj, self._positional_metadata_constructor_(0))
            self.assertReallyEqual(
                obj,
                self._positional_metadata_constructor_(0, positional_metadata=empty),
            )

        for empty in None, pd.DataFrame(index=["a", "b"]):
            obj = self._positional_metadata_constructor_(2, positional_metadata=empty)

            self.assertReallyEqual(obj, self._positional_metadata_constructor_(2))
            self.assertReallyEqual(
                obj,
                self._positional_metadata_constructor_(2, positional_metadata=empty),
            )
    ...
    def test_copy_positional_metadata_empty(self):
        ...
        assert_data_frame_almost_equal(
            obj._positional_metadata, pd.DataFrame(index=range(3))
        )
        self.assertIsNone(obj_copy._positional_metadata)

    def test_copy_with_positional_metadata(self):
        obj = self._positional_metadata_constructor_(
            4, positional_metadata={"bar": [[], [], [], []], "baz": [42, 42, 42, 42]}
        )
        obj_copy = copy.copy(obj)
        ...
        self.assertIsNot(obj._positional_metadata, obj_copy._positional_metadata)
        self.assertIsNot(
            obj._positional_metadata.values, obj_copy._positional_metadata.values
        )
        self.assertIs(
            obj._positional_metadata.loc[0, "bar"],
        ...
        assert_data_frame_almost_equal(
            ...
            pd.DataFrame({"bar": [[1], [], [], []], "baz": [43, 42, 42, 42]}),
        )
        assert_data_frame_almost_equal(
            obj.positional_metadata,
            pd.DataFrame({"bar": [[1], [], [], []], "baz": [42, 42, 42, 42]}),
        )

    def test_copy_preserves_range_index(self):
        for pm in None, {"foo": ["a", "b", "c"]}:
            obj = self._positional_metadata_constructor_(3, positional_metadata=pm)
            obj_copy = copy.copy(obj)

            self.assertIsInstance(obj.positional_metadata.index, pd.RangeIndex)
            self.assertIsInstance(obj_copy.positional_metadata.index, pd.RangeIndex)
    ...
    def test_deepcopy_with_positional_metadata(self):
        ...
        assert_data_frame_almost_equal(
            obj.positional_metadata,
            pd.DataFrame({"bar": [[], [], [], []], "baz": [42, 42, 42, 42]}),
        )
    ...
    def test_positional_metadata_getter(self):
        ...
        self.assertIsInstance(obj.positional_metadata.index, pd.RangeIndex)
        assert_data_frame_almost_equal(
            obj.positional_metadata, pd.DataFrame({"foo": [22, 22, 0]})
        )

        # Update existing column.
        obj.positional_metadata["foo"] = [42, 42, 43]
        assert_data_frame_almost_equal(
            obj.positional_metadata, pd.DataFrame({"foo": [42, 42, 43]})
        )
        ...
        assert_data_frame_almost_equal(
            obj.positional_metadata,
            pd.DataFrame({"foo": [42, 42, 43], "foo2": [True, False, True]}),
        )
    ...
    def test_positional_metadata_getter_set_column_series(self):
        ...
        npt.assert_equal(obj.positional_metadata["baz"], np.array(range(length)))
    ...
    def test_positional_metadata_setter_none(self):
        obj = self._positional_metadata_constructor_(0, positional_metadata={"foo": []})

        self.assertTrue(obj.has_positional_metadata())
        assert_data_frame_almost_equal(
            obj.positional_metadata, pd.DataFrame({"foo": []})
        )
    ...
    def test_positional_metadata_deleter(self):
        ...
        self.assertIsInstance(obj.positional_metadata.index, pd.RangeIndex)
        assert_data_frame_almost_equal(
            obj.positional_metadata, pd.DataFrame({"foo": [1, 2, 3]})
        )
...
class IntervalMetadataMixinTests:
    def _set_up(self):
        ...
        self.intvls = [
            ...
            {"bounds": [(0, 1)], "metadata": {"gene": ["a"], "product": "foo"}},
        ]

    def test_constructor_invalid(self):
        with self.assertRaisesRegex(
            TypeError, "You must provide `IntervalMetadata` " "object."
        ):
    ...
    def test_copy_interval_metadata(self):
        ...
        self.assertIsNot(obj.interval_metadata, obj_copy.interval_metadata)
        self.assertIsNot(
            obj.interval_metadata._intervals, obj_copy.interval_metadata._intervals
        )
        for i, j in zip(
            obj.interval_metadata._intervals, obj_copy.interval_metadata._intervals
        ):
