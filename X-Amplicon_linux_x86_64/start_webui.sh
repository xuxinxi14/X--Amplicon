...
Start the local X-Amplicon Web UI on Linux.
...
Options:
  ...
  --repair-deps         Run setup_linux.sh before starting.
...
if [[ -x "$ROOT_DIR/.venv/bin/python" ]]; then
  ...
  fail "Python was not found. Run ./setup_linux.sh first."
fi
...
if ! "$PYTHON_EXE" -c "import fastapi, uvicorn" >/dev/null 2>&1; then
  ...
  printf 'Install them with: ./setup_linux.sh\n' >&2
  exit 1
fi
